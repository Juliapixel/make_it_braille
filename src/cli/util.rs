...
pub fn try_get_from_url(url: Url) -> Result<image::Frames<'static>, Error> {
    ...
    let format = format_header.and_then(|h| {
        image::ImageFormat::from_mime_type(h.to_str().ok()?)
    });

    load_as_frames(payload.clone(), format)
}

pub fn load_as_frames(data: impl AsRef<[u8]> + 'static, format: Option<ImageFormat>) -> Result<image::Frames<'static>, Error> {
    match format {
        ...
        Some(f) => {
            ...
            Ok(image::Frames::new(Box::new([Ok(image::Frame::new(frame.into_rgba8()))].into_iter())))
        }
        None => {
